...
// ─── Helpers ───
function ok(data: any = null, message = 'success') {
  return { code: 0, message, data }
}

function mockToken(userId: number, role: string) {
  return Buffer.from(JSON.stringify({ id: userId, role, exp: Date.now() + 86400000 })).toString('base64')
}

function paginate(arr: any[], page: number, pageSize: number) {
...
// ─── Route Handler ───
function handleApi(method: string, path: string, body: any, query: Record<string, string>) {
  ...
  if (method === 'POST' && path === '/api/v1/auth/login') {
    ...
    if (!username || !password) return { code: 400, message: '用户名和密码不能为空', data: null }
    const user = users.find(u => u.username === username)
    if (!user) return { code: 401, message: '用户名或密码错误', data: null }
    return ok({ token: mockToken(user.id, user.role), refreshToken: mockToken(user.id, user.role) + '_refresh', user: { id: user.id, username: user.username, role: user.role } })
  }
  if (method === 'POST' && path === '/api/v1/auth/register') {
    const { username, role } = body
    if (users.find(u => u.username === username)) return { code: 400, message: '用户名已存在', data: null }
    const newUser = { id: users.length + 1, username, role: role || 'viewer' }
    users.push(newUser)
    return ok({ token: mockToken(newUser.id, newUser.role), refreshToken: mockToken(newUser.id, newUser.role) + '_refresh', user: { id: newUser.id, username: newUser.username, role: newUser.role } })
  }
  if (method === 'POST' && path === '/api/v1/auth/refresh') {
    const user = users[0]
    return ok({ token: mockToken(user.id, user.role), refreshToken: mockToken(user.id, user.role) + '_refresh', user: { id: user.id, username: user.username, role: user.role } })
  }
  if (method === 'PUT' && path === '/api/v1/auth/password') return ok(null, '密码修改成功')
  if (method === 'GET' && path === '/api/v1/auth/users') return ok(users)
  if (method === 'GET' && path.match(/^\/api\/v1\/auth\/users\/\d+$/)) {
    const id = parseInt(path.split('/').pop()!)
    return ok(users.find(u => u.id === id) || null)
  }
  if (method === 'PUT' && path.match(/^\/api\/v1\/auth\/users\/\d+$/)) {
    const id = parseInt(path.split('/').pop()!)
  ...
  if (method === 'GET' && path === '/api/v1/devices') {
    ...
    if (query.keyword) filtered = filtered.filter(d => d.name.includes(query.keyword) || d.location.includes(query.keyword))
    return ok(paginate(filtered, page, pageSize))
  }
  if (method === 'GET' && path.match(/^\/api\/v1\/devices\/\d+$/)) {
    const id = parseInt(path.split('/').pop()!)
    return ok(devices.find(d => d.id === id) || null)
  }
  if (method === 'POST' && path === '/api/v1/devices') {
    const newDev = { id: devices.length + 1, ...body, status: 'offline', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }
    devices.push(newDev)
    return ok(newDev)
  }
  if (method === 'PUT' && path.match(/^\/api\/v1\/devices\/\d+$/) && !path.includes('/status') && !path.includes('/start') && !path.includes('/stop')) {
    const id = parseInt(path.split('/')[4])
    const dev = devices.find(d => d.id === id)
    if (dev) Object.assign(dev, body, { updatedAt: new Date().toISOString() })
    return ok(dev)
  }
  if (method === 'GET' && path.match(/^\/api\/v1\/devices\/\d+\/status$/)) {
    const id = parseInt(path.split('/')[4])
    const dev = devices.find(d => d.id === id)
    return ok({ status: dev?.status || 'offline', fps: dev?.fps || 0, bitrate: Math.floor(Math.random() * 8000 + 2000) })
  }
  if (method === 'POST' && path.match(/^\/api\/v1\/devices\/\d+\/start$/)) {
    const id = parseInt(path.split('/')[4])
    const dev = devices.find(d => d.id === id)
    if (dev) dev.status = 'online'
    return ok(null, '启动成功')
  }
  if (method === 'POST' && path.match(/^\/api\/v1\/devices\/\d+\/stop$/)) {
    const id = parseInt(path.split('/')[4])
    const dev = devices.find(d => d.id === id)
    if (dev) dev.status = 'offline'
    return ok(null, '停止成功')
  }
  if (method === 'DELETE' && path.match(/^\/api\/v1\/devices\/\d+$/)) {
    const id = parseInt(path.split('/').pop()!)
  ...
  // ─── Algorithms ───
  if (method === 'GET' && path === '/api/v1/algorithms') {
    ...
    if (query.keyword) filtered = filtered.filter(a => a.name.includes(query.keyword))
    return ok(paginate(filtered, page, pageSize))
  }
  if (method === 'GET' && path === '/api/v1/algorithms/types') return ok(['detection', 'tracking', 'classification', 'segmentation', 'pose', 'ocr', 'face'])
  if (method === 'GET' && path.match(/^\/api\/v1\/algorithms\/\d+$/)) {
    const id = parseInt(path.split('/').pop()!)
    return ok(algorithms.find(a => a.id === id) || null)
  }
  if (method === 'POST' && path === '/api/v1/algorithms') {
    const newAlgo = { id: algorithms.length + 1, ...body, status: 'inactive', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }
    algorithms.push(newAlgo)
    return ok(newAlgo)
  }
  if (method === 'PUT' && path.match(/^\/api\/v1\/algorithms\/\d+$/)) {
    const id = parseInt(path.split('/')[4])
    const algo = algorithms.find(a => a.id === id)
    if (algo) Object.assign(algo, body, { updatedAt: new Date().toISOString() })
    return ok(algo)
  }
  if (method === 'DELETE' && path.match(/^\/api\/v1\/algorithms\/\d+$/)) {
    const id = parseInt(path.split('/').pop()!)
    algorithms = algorithms.filter(a => a.id !== id)
    return ok(null, '删除成功')
  }
  if (method === 'POST' && path.match(/^\/api\/v1\/algorithms\/\d+\/validate$/)) {
    return ok({ valid: true, message: '模型验证通过' })
  }
  ...
  if (method === 'GET' && path === '/api/v1/tasks') {
    ...
    if (query.keyword) filtered = filtered.filter(t => t.name.includes(query.keyword))
    return ok(paginate(filtered, page, pageSize))
  }
  if (method === 'GET' && path.match(/^\/api\/v1\/tasks\/\d+$/)) {
    const id = parseInt(path.split('/').pop()!)
    return ok(tasks.find(t => t.id === id) || null)
  }
  if (method === 'POST' && path === '/api/v1/tasks') {
    const newTask = { id: tasks.length + 1, ...body, status: 'stopped', fps: 0, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }
    tasks.push(newTask)
    return ok(newTask)
  }
  if (method === 'PUT' && path.match(/^\/api\/v1\/tasks\/\d+\/config$/)) {
    const id = parseInt(path.split('/')[4])
    const task = tasks.find(t => t.id === id)
    if (task) task.config = { ...task.config, ...body }
    return ok(null, '配置已更新')
  }
  if (method === 'PUT' && path.match(/^\/api\/v1\/tasks\/\d+\/roi$/)) {
    const id = parseInt(path.split('/')[4])
    const task = tasks.find(t => t.id === id)
    if (task) task.roi = body
    return ok(null, 'ROI已更新')
  }
  if (method === 'PUT' && path.match(/^\/api\/v1\/tasks\/\d+$/)) {
    const id = parseInt(path.split('/')[4])
    const task = tasks.find(t => t.id === id)
    if (task) Object.assign(task, body, { updatedAt: new Date().toISOString() })
    return ok(task)
  }
  if (method === 'POST' && path.match(/^\/api\/v1\/tasks\/\d+\/start$/)) {
    const id = parseInt(path.split('/')[4])
    const task = tasks.find(t => t.id === id)
    if (task) { task.status = 'running'; task.fps = +(Math.random() * 30 + 10).toFixed(1) }
    return ok(null, '任务已启动')
  }
  if (method === 'POST' && path.match(/^\/api\/v1\/tasks\/\d+\/stop$/)) {
    const id = parseInt(path.split('/')[4])
    const task = tasks.find(t => t.id === id)
    if (task) { task.status = 'stopped'; task.fps = 0 }
    return ok(null, '任务已停止')
  }
  if (method === 'POST' && path.match(/^\/api\/v1\/tasks\/\d+\/pause$/)) {
    const id = parseInt(path.split('/')[4])
    const task = tasks.find(t => t.id === id)
    if (task) { task.status = 'paused'; task.fps = 0 }
    return ok(null, '任务已暂停')
  }
  if (method === 'POST' && path.match(/^\/api\/v1\/tasks\/\d+\/resume$/)) {
    const id = parseInt(path.split('/')[4])
    const task = tasks.find(t => t.id === id)
    if (task) { task.status = 'running'; task.fps = +(Math.random() * 30 + 10).toFixed(1) }
    return ok(null, '任务已恢复')
  }
  if (method === 'GET' && path.match(/^\/api\/v1\/tasks\/\d+\/metrics$/)) {
    return ok({ fps: +(Math.random() * 30 + 10).toFixed(1), latency: +(Math.random() * 50 + 5).toFixed(1), throughput: +(Math.random() * 100 + 50).toFixed(0), detections: Math.floor(Math.random() * 20 + 1), timestamp: new Date().toISOString() })
  }
  if (method === 'DELETE' && path.match(/^\/api\/v1\/tasks\/\d+$/)) {
    const id = parseInt(path.split('/').pop()!)
  ...
  // ─── Monitor ───
  if (method === 'GET' && path === '/api/v1/monitor/system') {
    return ok({ cpuUsage: +(Math.random() * 40 + 30).toFixed(1), memoryUsage: +(Math.random() * 30 + 40).toFixed(1), memoryTotal: 16384, memoryUsed: Math.floor(Math.random() * 4096 + 6144), tpuUsage: +(Math.random() * 50 + 20).toFixed(1), gpuUsage: +(Math.random() * 30 + 10).toFixed(1), uptime: Math.floor(Date.now() / 1000 - 86400 * 3), timestamp: new Date().toISOString() })
  }
  if (method === 'GET' && path.match(/^\/api\/v1\/monitor\/tasks\/\d+$/)) {
    const id = parseInt(path.split('/').pop()!)
    return ok({ taskId: id, taskName: '示例任务', fps: +(Math.random() * 30 + 10).toFixed(1), latency: +(Math.random() * 50 + 5).toFixed(1), throughput: +(Math.random() * 100 + 50).toFixed(0), detections: Math.floor(Math.random() * 20 + 1) })
  }
  if (method === 'GET' && path.match(/^\/api\/v1\/monitor\/devices\/\d+$/)) {
    const id = parseInt(path.split('/').pop()!)
    const dev = devices.find(d => d.id === id)
    return ok({ deviceId: id, deviceName: dev?.name || '未知设备', status: dev?.status || 'offline', fps: dev?.fps || 0, bitrate: Math.floor(Math.random() * 8000 + 2000) })
  }
  ...
  if (method === 'GET' && path === '/api/v1/dashboard') {
    ...
    const taskStatusDistribution = { running: tasks.filter(t => t.status === 'running').length, stopped: tasks.filter(t => t.status === 'stopped').length, paused: tasks.filter(t => t.status === 'paused').length, error: tasks.filter(t => t.status === 'error').length }
    const alertTrend = []
    for (let i = 6; i >= 0; i--) {
      const d = new Date()
      d.setDate(d.getDate() - i)
      alertTrend.push({ date: d.toISOString().split('T')[0], count: Math.floor(Math.random() * 20 + 5) })
    }
    return ok({
      totalDevices: devices.length, onlineDevices, runningTasks, todayAlerts,
      systemMetrics: { cpuUsage: +(Math.random() * 40 + 30).toFixed(1), memoryUsage: +(Math.random() * 30 + 40).toFixed(1), memoryTotal: 16384, memoryUsed: Math.floor(Math.random() * 4096 + 6144), tpuUsage: +(Math.random() * 50 + 20).toFixed(1), gpuUsage: +(Math.random() * 30 + 10).toFixed(1), uptime: Math.floor(Date.now() / 1000 - 86400 * 3), timestamp: new Date().toISOString() },
      recentAlerts: alerts.slice(0, 5), taskStatusDistribution, alertTrend,
    })
  }
  ...
  if (method === 'GET' && path === '/api/v1/alerts') {
    ...
    if (query.acknowledged !== undefined) filtered = filtered.filter(a => a.acknowledged === (query.acknowledged === 'true'))
    return ok(paginate(filtered, page, pageSize))
  }
  if (method === 'PUT' && path.match(/^\/api\/v1\/alerts\/\d+\/acknowledge$/)) {
    const id = parseInt(path.split('/')[4])
    const alert = alerts.find(a => a.id === id)
    if (alert) { alert.acknowledged = true; alert.acknowledgedBy = 'admin' }
    return ok(null, '已确认报警')
  }
  if (method === 'POST' && path === '/api/v1/alerts/batch-acknowledge') {
    const { ids } = body
    ids.forEach((id: number) => { const a = alerts.find(x => x.id === id); if (a) { a.acknowledged = true; a.acknowledgedBy = 'admin' } })
    return ok(null, '批量确认成功')
  }
  if (method === 'GET' && path === '/api/v1/alerts/stats') {
    return ok({ total: alerts.length, info: alerts.filter(a => a.level === 'info').length, warning: alerts.filter(a => a.level === 'warning').length, critical: alerts.filter(a => a.level === 'critical').length, unacknowledged: alerts.filter(a => !a.acknowledged).length })
  }
  if (method === 'GET' && path === '/api/v1/alerts/rules') {
    return ok(paginate(alertRules, page, pageSize))
  }
  if (method === 'POST' && path === '/api/v1/alerts/rules') {
    const newRule = { id: alertRules.length + 1, ...body, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }
    alertRules.push(newRule)
    return ok(newRule)
  }
  if (method === 'PUT' && path.match(/^\/api\/v1\/alerts\/rules\/\d+$/)) {
    const id = parseInt(path.split('/').pop()!)
    const rule = alertRules.find(r => r.id === id)
    if (rule) Object.assign(rule, body, { updatedAt: new Date().toISOString() })
    return ok(rule)
  }
  if (method === 'DELETE' && path.match(/^\/api\/v1\/alerts\/rules\/\d+$/)) {
    const id = parseInt(path.split('/').pop()!)
  ...
  if (method === 'GET' && path.match(/^\/api\/v1\/plugins\/\d+$/)) {
    const id = parseInt(path.split('/').pop()!)
    return ok(plugins.find(p => p.id === id) || null)
  }
  if (method === 'POST' && path === '/api/v1/plugins/install') {
    const newPlugin = { id: plugins.length + 1, name: '新插件', version: '1.0.0', type: 'element', description: '新安装的插件', path: body.path || '/plugins/new.so', status: 'inactive', config: {}, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }
    plugins.push(newPlugin)
    return ok(newPlugin)
  }
  if (method === 'PUT' && path.match(/^\/api\/v1\/plugins\/\d+$/)) {
    const id = parseInt(path.split('/')[4])
    const plugin = plugins.find(p => p.id === id)
    if (plugin) Object.assign(plugin, body, { updatedAt: new Date().toISOString() })
    return ok(plugin)
  }
  if (method === 'DELETE' && path.match(/^\/api\/v1\/plugins\/\d+$/)) {
    const id = parseInt(path.split('/').pop()!)
    plugins = plugins.filter(p => p.id !== id)
    return ok(null, '删除成功')
  }
  if (method === 'POST' && path.match(/^\/api\/v1\/plugins\/\d+\/activate$/)) {
    const id = parseInt(path.split('/')[4])
    const plugin = plugins.find(p => p.id === id)
    if (plugin) plugin.status = 'active'
    return ok(null, '插件已激活')
  }
  if (method === 'POST' && path.match(/^\/api\/v1\/plugins\/\d+\/deactivate$/)) {
    const id = parseInt(path.split('/')[4])
    const plugin = plugins.find(p => p.id === id)
    if (plugin) plugin.status = 'inactive'
    return ok(null, '插件已停用')
  }

  // ─── System ───
  if (method === 'GET' && path === '/api/v1/system/info') {
    return ok({ version: '1.0.0', uptime: Math.floor(Date.now() / 1000 - 86400 * 3), cpuModel: 'Sophon BM1684X', cpuCores: 8, memoryTotal: 16384, tpuModel: 'BM1684X', tpuCount: 1, os: 'Linux 5.4.0-aarch64', hostname: 'sophon-edge-01' })
  }
  if (method === 'GET' && path === '/api/v1/system/config') return ok(systemConfig)
  if (method === 'PUT' && path === '/api/v1/system/config') { Object.assign(systemConfig, body); return ok(systemConfig, '配置已更新') }
  if (method === 'GET' && path === '/api/v1/system/audit-logs') {
  ...
  if (method === 'POST' && path === '/api/v1/system/backup') return ok({ path: `/data/backups/backup_${Date.now()}.tar.gz` })
  if (method === 'POST' && path === '/api/v1/system/restore') return ok(null, '系统已恢复')
  if (method === 'GET' && path === '/api/v1/system/firmware') return ok(firmwareVersions)
  if (method === 'POST' && path.match(/^\/api\/v1\/system\/firmware\/\d+\/install$/)) {
    const id = parseInt(path.split('/')[5])
    const fw = firmwareVersions.find(f => f.id === id)
    if (fw) { fw.status = 'installed'; fw.installedAt = new Date().toISOString() }
    return ok(null, '固件安装成功')
  }
  if (method === 'POST' && path === '/api/v1/system/reboot') return ok(null, '系统将重启')
  if (method === 'GET' && path === '/api/v1/health') return ok({ status: 'ok', version: '1.0.0' })
    ...
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        ...
        if (method === 'GET' || method === 'DELETE') {
          const result = handleApi(method, path, null, query)
        ...
        req.on('end', () => {
          try {
            ...
            const result = handleApi(method, path, parsed, query)
            res.setHeader('Content-Type', 'application/json')
            res.setHeader('Access-Control-Allow-Origin', '*')
            res.end(JSON.stringify(result))
          } catch (e) {
            res.setHeader('Content-Type', 'application/json')
            res.end(JSON.stringify({ code: 500, message: '请求解析失败', data: null }))
          }
        })
      })
