...
int getId(const drogon::HttpRequestPtr& req, const Json::Value* body=nullptr) {
    std::string v = req->getParameter("id");
    if (!v.empty()) try { return std::stoi(v); } catch (...) {}
    if (body && body->isMember("id")) try { return (*body)["id"].asInt(); } catch (...) {}
    return 0;
}
...
void AlertCtrl::asyncHandleHttpRequest(const drogon::HttpRequestPtr& req, std::function<void(const drogon::HttpResponsePtr&)>&& cb) {
    try {
        ...
        if(path=="/api/v1/alert-rules"&&method==drogon::Put) {
            auto body=req->getJsonObject();int id=getId(req,body.get());if(id==0||!body){cb(errResp(400,"Invalid"));return;}
        ...
        if(path=="/api/v1/alert-rules"&&method==drogon::Delete) {
            int id=getId(req);auto body=req->getJsonObject();if(id==0&&body&&body->isMember("id"))try{id=(*body)["id"].asInt();}catch(...){}
            if(id==0){cb(errResp(400,"Invalid id"));return;}auto r=db.remove("alert_rules","id=?", {std::to_string(id)});cb(jsonResp(0,r.success?"success":"Failed"));return;
        }
        cb(errResp(404,"Not found"));
