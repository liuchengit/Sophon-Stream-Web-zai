...
void DeviceCtrl::asyncHandleHttpRequest(const drogon::HttpRequestPtr& req, std::function<void(const drogon::HttpResponsePtr&)>&& cb) {
    try {
        ...
        if(path=="/api/v1/devices") {
            int id=getId(req);
            if(id>0) {
                if(method==drogon::Get) {auto r=db.findById("devices",id);if(r.success&&!r.rows.empty()){const auto&w=r.rows[0];nlohmann::json o;o["id"]=std::stoi(w.at("id"));o["name"]=w.at("name");o["type"]=w.at("type");o["streamUrl"]=w.at("stream_url");o["status"]=statusToStr(std::stoi(w.at("status")));o["location"]=w.at("location");o["description"]=w.at("description");cb(jsonResp(0,"success",o));}else{cb(errResp(404,"Not found",drogon::k404NotFound));}return;}
            ...
            if(method==drogon::Get) {auto r=db.find("devices");nlohmann::json a=nlohmann::json::array();for(const auto&w:r.rows){nlohmann::json o;o["id"]=std::stoi(w.at("id"));o["name"]=w.at("name");o["type"]=w.at("type");o["streamUrl"]=w.at("stream_url");o["protocol"]=w.at("protocol");o["resolution"]=w.at("resolution");o["fps"]=std::stoi((w.at("fps")));o["codec"]=w.at("codec");o["status"]=statusToStr(std::stoi(w.at("status")));o["location"]=w.at("location");o["description"]=w.at("description");o["deviceId"]=w.at("device_id");o["createdAt"]=w.at("created_at");o["updatedAt"]=w.at("updated_at");a.push_back(o);}cb(jsonResp(0,"success",{{"items",a},{"total",(int64_t)a.size()}}));return;}
